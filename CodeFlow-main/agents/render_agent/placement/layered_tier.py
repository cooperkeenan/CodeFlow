...
def place(template: DiagramTemplate) -> list[dict]:
    tier_indices: dict[str, int] = template.meta.get("tier_indices", {})
    by_rank: dict[int, list] = {}
    for node in template.nodes:
        rank = tier_indices.get(node.module_name, 0)
        by_rank.setdefault(rank, []).append(node)

    result = []
    for row_idx, rank in enumerate(sorted(by_rank)):
        row = by_rank[rank]
...
def place_component(template: DiagramTemplate) -> list[dict]:
    depth_map: dict[str, int] = template.meta.get("depth_map", template.meta.get("tier_indices", {}))
    by_rank: dict[int, list] = {}
    for node in template.nodes:
        rank = depth_map.get(node.id, 0)
        by_rank.setdefault(rank, []).append(node)
